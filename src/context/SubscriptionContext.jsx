...
export function SubscriptionProvider({ children }) {
  ...
  const refreshSubscription = useCallback(async ({ preserveRetryState = false, bypassCache = false } = {}) => {
    ...
    try {
      ...
      const resolvedStripeTier = normalizeTier(
        stripeSubscription?.plan || stripeSubscription?.tier || stripeResult.plan || stripeResult.tier
      );
      const nextTier = hasActiveSubscription
        ? (databaseTier || resolvedStripeTier || TIERS.FREE)
        : TIERS.FREE;
      const resolvedSubscriptionTier = hasTerminalStripeStatus
        ? TIERS.FREE
