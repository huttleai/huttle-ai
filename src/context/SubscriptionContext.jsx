...
export function SubscriptionProvider({ children }) {
  ...
  const refreshSubscription = useCallback(async ({ preserveRetryState = false, bypassCache = false } = {}) => {
    ...
    try {
      ...
      const liveStripeStatus = stripeSubscription?.status || stripeResult.status || null;
      const isLiveStripeNoAccess = liveStripeStatus && NO_ACCESS_STRIPE_STATUSES.has(liveStripeStatus);
      const nextStatus = isLiveStripeNoAccess
        ? liveStripeStatus
        : (stripeSubscription?.status || databaseSubscription?.status || stripeResult.status || 'inactive');
      ...
      const nextSubscription = stripeSubscription
        ...
        : (databaseSubscription
          ? {
              ...
              plan: isLiveStripeNoAccess ? TIERS.FREE : databaseTier,
              tier: isLiveStripeNoAccess ? TIERS.FREE : databaseTier,
              status: nextStatus,
            }
          : null);
