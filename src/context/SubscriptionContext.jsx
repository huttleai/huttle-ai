...
export function SubscriptionProvider({ children }) {
  ...
  const refreshSubscription = useCallback(async ({ preserveRetryState = false, bypassCache = false } = {}) => {
    ...
    try {
      ...
      const shouldTrustStripeStatus =
        stripeStatus &&
        !stripeResult?.degraded &&
        (stripeResult?.success || TERMINAL_ACCESS_STATUSES.has(stripeStatus));
      const nextStatus = shouldTrustStripeStatus
        ? stripeStatus
