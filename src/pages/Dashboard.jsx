...
export default function Dashboard() {
  ...
  const dashboardHashtags = useMemo(
    () => (Array.isArray(dashboardData?.hashtags_of_day) ? dashboardData.hashtags_of_day.slice(0, 8) : []),
    [dashboardData]
  );
                    ...
                    >
                      <div className="p-3">
                        <div className="flex items-start justify-between gap-2">
                          <p className="font-semibold text-sm text-gray-900 truncate">{trend.topic}</p>
                      ...
                      {isExpanded && (
                        <div className="border-t border-gray-100 px-3 pb-3 pt-3 bg-gray-50/50 rounded-b-xl animate-fadeIn space-y-3">
                          {trend.description && (
                            <p className="text-xs text-gray-700 leading-relaxed">{trend.description}</p>
                          )}
                          {Array.isArray(trend.content_angles) && trend.content_angles.length > 0 && (
                            <div>
                              <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wide mb-1.5">Content Angles</p>
                              <ul className="space-y-1">
                                {trend.content_angles.slice(0, 3).map((angle, ai) => (
                                  <li key={ai} className="text-xs text-gray-700 flex items-start gap-1.5">
                                    <span className="text-huttle-primary mt-0.5">•</span>
                                    <span>{angle}</span>
                                  </li>
                                ))}
                              </ul>
                          ...
                          <div className="flex items-center gap-2 pt-1">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(`/dashboard/ai-tools?topic=${encodeURIComponent(trend.topic)}`);
                              }}
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(`/dashboard/trend-lab?topic=${encodeURIComponent(trend.topic)}`);
                              }}
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold border border-gray-200 text-gray-700 bg-white rounded-lg hover:bg-gray-50 transition-colors"
                            >
