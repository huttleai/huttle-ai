...
import { resolveAuthenticatedUserId } from '../_utils/billing.js';
...
export default async function handler(req, res) {
  ...
  try {
    const authResult = await resolveAuthenticatedUserId({
      ...
      mismatchError: 'You can only trigger usage alerts for your own account',
    });
    ...
    if (sendResult?.error) {
      console.error('Usage alert email send error:', sendResult.error);
      return res.status(502).json({ error: 'Failed to send usage alert' });
    }
    ...
    if (activityError) {
      console.error('Usage alert activity insert error:', activityError);
      return res.status(500).json({ error: 'Failed to mark usage alert as sent' });
    }

    return res.status(200).json({ sent: true, planName, creditResetDate, daysUntilReset });
  } catch (err) {
    console.error('Usage alert trigger failed:', err);
    return res.status(500).json({ error: 'Failed to trigger usage alert' });
  }
}
