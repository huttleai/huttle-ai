...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (alreadySentError) {
      throw new Error(alreadySentError.message || 'Failed to check alert status');
    }
    ...
    if (authUserError) {
      throw new Error(authUserError.message || 'Failed to load user email');
    }
    ...
    if (insertError) {
      throw new Error(insertError.message || 'Failed to mark alert as sent');
    }
