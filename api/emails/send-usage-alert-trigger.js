...
export default async function handler(req, res) {
  ...
  if (requestedUserId && requestedUserId !== userId) {
    return res.status(403).json({ error: 'You can only send alerts for your own account' });
  }
  ...
  try {
    ...
    if (sendResult?.error) {
      throw new Error(sendResult.error.message || 'Failed to send usage alert email');
    }
