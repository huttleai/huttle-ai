 ...
 *
 * Auth: Supabase bearer token for the user whose alert may be sent.
 * Body: { userId?: string } for an optional backwards-compatible echo guard.
 *
...
export default async function handler(req, res) {
  ...
  if (requestedUserId && requestedUserId !== user.id) {
    return res.status(403).json({ error: 'Cannot send usage alert for another user' });
  }
