    ...
    switch (event.type) {
        ...
        try {
          ...
          if (!userId && customerEmail) {
            ...
            if (userLookupError) {
              ...
              throw new Error(userLookupError.message || 'Failed to resolve checkout user');
            }
          ...
          if (!userId) {
            ...
            throw new Error(`Could not resolve Supabase user for checkout session ${session.id}`);
          }
      ...
      case 'customer.subscription.updated': {
        try {
          ...
          if (profileError && profileError.code !== 'PGRST116') {
            ...
            throw new Error(profileError.message || 'Failed to resolve subscription user');
          }
            ...
            try {
              ...
              if (!existingSub) {
                ...
                if (statusSyncError) {
                  ...
                  throw new Error(statusSyncError.message || 'Failed to sync subscription status');
                } else {
      ...
      case 'customer.subscription.deleted': {
        try {
          ...
          let resolvedUserId = profile?.user_id || subscriptionRow?.user_id || subscription.metadata?.supabase_user_id || null;
          ...
          if (!resolvedUserId) {
            try {
              ...
              logWarn('stripe_webhook.subscription_deleted_customer_metadata_failed', {
                eventId: event.id,
                customerId,
                error: custErr.message,
              });
          ...
          if (resolvedUserId || stripeSubId) {
            ...
            let { error: delError } = await supabase
              ...
              .eq(resolvedUserId ? 'user_id' : 'stripe_subscription_id', resolvedUserId || stripeSubId);

            if (delError?.message?.toLowerCase().includes('cancelled_at')) {
              const { cancelled_at: _ca, ...fallback } = deletionPayload;
              delError = (await supabase
                .from('subscriptions')
                .update(fallback)
                .eq(resolvedUserId ? 'user_id' : 'stripe_subscription_id', resolvedUserId || stripeSubId)).error;
            }

            if (delError) {
              logError('stripe_webhook.subscription_deleted_update_failed', { eventId: event.id, userId: resolvedUserId, subscriptionId: stripeSubId, error: delError.message });
            ...
            logError('stripe_webhook.subscription_deleted_no_user_found', {
              eventId: event.id,
              customerId,
              subscriptionId: stripeSubId,
            });
            throw new Error(`Could not resolve deleted subscription ${stripeSubId}`);
          }
        } catch (err) {
      ...
      case 'invoice.payment_failed': {
        try {
          ...
          if (profile) {
            const { error: paymentFailedStatusError } = await supabase
              .from('subscriptions')
              .update({
                status: 'past_due',
                updated_at: new Date().toISOString(),
            ...
            if (paymentFailedStatusError) {
              logError('stripe_webhook.invoice_payment_failed_status_update_failed', {
                eventId: event.id,
                userId: profile.user_id,
                error: paymentFailedStatusError.message,
              });
              throw new Error(paymentFailedStatusError.message || 'Failed to sync payment failure status');
            }
        ...
        try {
          ...
          if (profile && invoice.subscription) {
            ...
            if (paidError) {
              ...
              throw new Error(paidError.message || 'Failed to sync paid invoice status');
            }
          }
        } catch (err) {
