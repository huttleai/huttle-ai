    ...
    switch (event.type) {
      ...
      case 'customer.subscription.updated': {
        try {
          ...
          if (profileError && profileError.code !== 'PGRST116') {
            ...
            throw new Error(profileError.message || 'Failed to resolve subscription customer profile');
          }
          ...
          if (resolvedUserId) {
            ...
            throw new Error(`Could not resolve Supabase user for subscription ${subscription.id}`);
          }
      ...
      case 'customer.subscription.deleted': {
        try {
          ...
          let resolvedUserId = profile?.user_id || subscription.metadata?.supabase_user_id || null;
          ...
          if (!resolvedUserId) {
            const { data: subscriptionRow } = await supabase
              ...
              .eq('stripe_subscription_id', subscription.id)
              .maybeSingle();
            resolvedUserId = subscriptionRow?.user_id || null;
          }
          ...
          if (!resolvedUserId) {
            throw new Error(`Could not resolve Supabase user for deleted subscription ${subscription.id}`);
          }
          ...
          if (!profile?.first_name) {
            const { data: nameProfile } = await supabase
              ...
              .eq('user_id', resolvedUserId)
              .maybeSingle();
            firstName = nameProfile?.first_name || firstName;
          }
        ...
        try {
          ...
          if (profile && invoice.subscription) {
            ...
            if (paidError) {
              ...
              throw new Error(paidError.message || 'Failed to sync paid invoice');
            }
          }
        } catch (err) {
