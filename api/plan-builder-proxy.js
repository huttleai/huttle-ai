...
function resolveRequestedTimePeriod(bodyPeriod, jobInput) {
  const candidates = [
    bodyPeriod,
    jobInput?.timePeriod,
    jobInput?.duration,
  ...
  const matchedPeriod = candidates.find((candidate) => ['7', '14'].includes(String(candidate)));
  return matchedPeriod ? String(matchedPeriod) : '7';
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (job.user_id !== user.id) {
      ...
      return res.status(403).json({ error: 'You can only run your own Plan Builder jobs', requestId });
    }

    if (job.type && job.type !== 'plan_builder') {
      return res.status(400).json({ error: 'Job is not a Plan Builder job', requestId });
    }

    const resolvedTimePeriod = resolveRequestedTimePeriod(timePeriod, job.input);
    ...
    const isFourteenDay = Number(resolvedTimePeriod) === 14;
    ...
    const { featureKey, cap } = resolvePlanBuilderCap(resolvedTimePeriod, userTier);
    if (cap == null || cap <= 0) {
      return res.status(403).json({
    ...
    const { count: existingJobCounter, error: existingCounterError } = await supabase
      .from('user_activity')
    ...
    if (existingCounterError) {
      logError('plan_builder_proxy.job_counter_lookup_failed', {
        requestId,
        userId: user.id,
        job_id,
        featureKey,
        error: existingCounterError.message,
      });
      return res.status(500).json({ error: 'Failed to verify Plan Builder usage', requestId });
    }

    if (!existingJobCounter) {
      ...
      const { error: usageTrackError } = await supabase.from('user_activity').insert({
        user_id: user.id,
        feature: featureKey,
        metadata: {
          type: 'run_counter',
      ...
      if (usageTrackError) {
        logError('plan_builder_proxy.usage_track_failed', {
          ...
          job_id,
          featureKey,
          error: usageTrackError.message,
        });
        return res.status(500).json({ error: 'Failed to reserve Plan Builder usage', requestId });
      }
    }
